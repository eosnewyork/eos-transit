...
export default class LedgerDataManager {
	async serialize(chainId: any, transaction: any, types?: any, api?: any): Promise<Buffer> {
		...
		for(const action of transaction.actions) {
			...
			// TODO: Need to find a more efficient way to cache these ABI's. This method will fetch the ABI from the network for each action. 
			var b = Serialize.serializeActionData(await api.getContract(action.account), action.account, action.name, action.data, null, null);

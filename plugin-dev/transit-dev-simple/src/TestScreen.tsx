...
import WAL, { Wallet, DiscoveryData } from 'eos-transit';
...
export class TestScreen extends Component<any, LoginScreenState> {
  ...
  async connect() {
    ...
    if (this.wallet) {
      ...
      this.wallet.connect().then(() => {
        ...
        this.wallet
          .discover({ pathIndexList: [0, 1, 2] })
          .then((discoveryData: DiscoveryData) => {
            // Normally we'd check the discovery data here.
            this.logIfDebugEnabled('Calling login()');
            this.logIfDebugEnabled(
              'Transit will try to fetch account info for this account'
            );
            this.wallet.login().then(() => {
              const auth = this.wallet;

              this.logIfDebugEnabled(
                'Login Completed. The account info fetched from chain was:'
              );
  ...
  render() {
    return (
      <div>
        <Toggle>XXX</Toggle>
        <br />
        <TestButton onClick={() => this.connect()}>
          Connect, Discover and Login
        </TestButton>
        <br />
        <TestButton onClick={() => this.runTxn()}>Run Transaction</TestButton>
        <br />
        {this.wallet.provider.showProvider && (
          <TestButton onClick={() => this.show()}>Show Provider</TestButton>
        )}
        <br />
        <br />
        <br />
  ...
  show() {
    return (
      this.wallet.provider.showProvider && this.wallet.provider.showProvider()
    );
  }

...
export const PinCreationScreen: React.FC<ProviderProps> = (props) => {
  ...
  const onEnterPin = () => {
    ...
    if (typeof onSelect === 'function') {
      onSelect({ pin, appname: props.providerName });
    }
  }
  ...
  const renderButton = () => {
    if (!REGEX.NULL_WHITESPACE.test(pins.pin1)
      ...
      if (props.hasError) {
        return <WalletListItemConnectButton onClick={() => onEnterPin()} >Reconnect</WalletListItemConnectButton>
      }
      else {
        return <GoButton onClick={() => onEnterPin()} ><>Go</></GoButton>
      }
    } else {
      if (props.hasError) {
        return <WalletListItemConnectButton disabled={true} style={{ background: '#bbb' }}>Reconnect</WalletListItemConnectButton>
      }
      else {
  ...
  return (
    <div>
      <Column>
        <Row>
          <input className={TextInput} type='password' maxLength={1} value={pins.pin1}
            name='pin1' ref={pin1}
            onChange={updateField} />
          <input className={TextInput} maxLength={1} value={pins.pin2} type='password'
            name='pin2' ref={pin2}
            onChange={updateField} />
          <input className={TextInput} maxLength={1} value={pins.pin3} type='password'
            name='pin3' ref={pin3}
            onChange={updateField} />
          <input className={TextInput} maxLength={1} value={pins.pin4} type='password'
            name='pin4' ref={pin4}
            onChange={updateField} />
        </Row>
        <Row>
